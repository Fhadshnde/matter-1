...
export default function AddRequestModal({ onSubmit }) {
  ...
  const handleSubmit = () => {
    // تنفيذ شيء مع البيانات مثل إرسال API
    if (onSubmit) onSubmit(formData);
    cancelModal();
  };

  // لمنع إغلاق المودال عند الضغط داخل المحتوى
  ...
  return (
    <>
      ...
      {openModal && (
        ...
        >
          <div
            className="bg-[#1A1A1A] rounded-2xl h-[90%] w-full border border-white/5 shadow-2xl modal-content-enter-active overflow-auto"
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-[#5E54F2] to-[#7C3AED] rounded-t-2xl flex items-center justify-between">
              <h3 className="text-xl font-bold text-white">اضافة طلب</h3>
              <button
                onClick={cancelModal}
